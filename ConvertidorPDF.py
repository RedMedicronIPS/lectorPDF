...
def procesar_seccion(texto):
    ...
    for linea in lineas:
        ...
        if not linea:
            continue
        if linea.isdigit() or linea.isalpha():
...
# Función para extraer y procesar los datos de los archivos PDF
def extraer_datos():
    if not carpeta_pdf:
    ...
    barra_progreso.start()  # Inicia la barra de progreso
    ...
    archivo_csv = os.path.join(carpeta_pdf, 'datos_extraidos.txt')
